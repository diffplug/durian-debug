 ...
 * ~10 ms after the previous one.  Useful for setting up a
 * blocking set of locks, and ensuring that they contend with
 * each other in the desired fashion.
 */
public class ThreadHarness {
	...
	public void run() {
		List<TestThread> testThreads = IntStream.range(0, allToRun.size())
				.mapToObj(i -> new TestThread(allToRun.get(i)))
	...
	static class TestThread extends Thread {
		...
		TestThread(Throwing.Runnable toRun) {
			this.toRun = toRun;
			start();
		}
